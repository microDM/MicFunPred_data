...
folders = ['3_predicted_97','4_predicted_98','5_predicted_99','6_predicted_100']
...
for i in datasets:
    try:
        df = pd.read_csv('2_humann2_metagenome/' + i + '_met.tsv.gz',sep='\t',index_col=0,compression='gzip')
        met_dict[i] = df
    except:
        pass
...
for i in folders:
    ...
    for j in os.listdir(i):
        ...
        df_met = met_dict[d]
        possible_ko_tool = list(pd.read_csv(os.path.join('possible_ko',tool+'.txt'),sep='\t',index_col=0,header=None).index)
        possible_ko_met = list(pd.read_csv(os.path.join('possible_ko','humann2.txt'),sep='\t',index_col=0,header=None).index)
